...
logger.debug(f"logger initialized {__name__}")
...
def run_as_threads():
    ...
    logger.info("...Stop tray icon")
    tray_icon.stop()
    logger.info("...Stop the Screenshot thread")
    stop_event.set()
    logger.info("Wait for the threads to terminate")
    t.join()
...
def shutdown_processes():
    logger.status("Terminating Processes")
    for process in processes:
        process.terminate()
        process.join()
...
if __name__ == "__main__":
    ...
    logger.debug(f"starting {app.name}")
    create_db()
    logger.status(f"Appdata folder: {appdata_folder}")
